...
module.exports.urlPrefixParameter = measure => (!soap.soapToMeasure(measure.code).parameters) ? '' : Object.values(soap.soapToMeasureWithWarnings(measure.code).parameters).map(p => `&${p.name}=${p.defaultValue}`).join('')
